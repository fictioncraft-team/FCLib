...
public class PacketUpdateClientBE {
    ...
    public void handle(Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            if (pos == null) {
                FCLibCore.LOGGER.error("Requested update but position is null");
                return;
            }
            ...
            if (level == null) {
                FCLibCore.LOGGER.error("Requested update at {} but level does not exist", pos);
                return;
            }
            ...
            if (te == null) {
                FCLibCore.LOGGER.error("Requested update at {} but no te is found", pos);
                return;
            }
